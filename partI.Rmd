...
model0.1 <- lm(formula = logprice ~ (dealer + year + origin_cat + artistliving +authorstyle + endbuyer + Interm + Surface + engraved + prevcoll + paired + finished + lrgfont + lands_sc + discauth), data = paint_train_trans)
#prepare for BIC
...
model1 <- lm(formula = logprice ~ dealer + year + origin_cat + artistliving + 
                   ...
                   + year:discauth + artistliving:authorstyle + artistliving:discauth + 
...
summary(g_interact)
options(scipen=999)
tb_final <- broom::tidy(g_interact, conf.level=0.95,
                   conf.int = TRUE, exponentiate=T)
new<-kable(tb_final, digits = 3,
      caption="Coefficient Summary for Final Model") %>%
  kable_styling(latex_options = "hold_position")

new.data <- paint_train%>%select(Surface,lrgfont, Interm, authorstyle,
    prevcoll,origin_cat,engraved,finished,discauth,
    dealer, lands_sc, endbuyer, year)%>%mutate(lrgfont=0, Interm=0, authorstyle=0,
    prevcoll=0,origin_cat='D/FL',engraved=0,finished=0,discauth=0,
    dealer='J', lands_sc=0, endbuyer='B')
predict(g_interact, newdata = new.data)
```

```{r}
ggplot() +
  geom_errorbar(data = new, aes(x = var, ymin = lwr, ymax = upr), size = 0.3) +
...
predictions = as.data.frame(
  exp(predict(g_interact, newdata=paint_test %>% mutate(Surface = log(Surface)), interval = "pred")))
