...
total.clean <- total %>% 
  ...
  #categorical
  mutate(winningbiddertype = as.factor(dplyr::recode(na_if(winningbiddertype,""),.missing = 'NA')),
         endbuyer = as.factor(dplyr::recode(na_if(endbuyer,""), .missing = 'X')),
         Interm = as.factor(dplyr::recode(as.character(total$Interm), .missing = "X")), #treat NA as X, there are some character NA and real NA, so transform to character first 
         type_intermed = as.factor(dplyr::recode(na_if(type_intermed,""), .missing = 'X')),
         Shape = as.factor(dplyr::recode(na_if(Shape, ""), .missing = 'X',
                                  `ovale` = 'oval',
                                  `ronde` = 'round')),
         materialCat = as.factor(dplyr::recode(na_if(materialCat,""), .missing = 'X')),
