...
paint_train_trans <- paint_train %>% 
  select(-c(price, othartist, mytho, relig, landsALL)) %>%
  mutate(Surface = log(Surface))

# use p-value to select the variables which could be used to build interaction terms
model.0 <- lm(logprice~., data = paint_train_trans)
summary(model.0)

model.1 = lm(logprice ~ . + (dealer + year + artistliving + authorstyle + winningbiddertype + 
               Surface + materialCat +  engraved + prevcoll + othartist +
               paired + lrgfont + portrait + still_life + discauth)^2 - 
               (dealer + year + artistliving + authorstyle + winningbiddertype + 
               Surface + materialCat +  engraved + prevcoll + othartist +
               paired + lrgfont + portrait + still_life + discauth),
             data = paint_train_trans)
summary(model.1)

model.1.aic = step(model.1, k = log(nrow(paint_train_trans)), trace = F)
summary(model.1.aic)
```
