...
paint %>% select(endbuyer, logprice) %>% filter(!is.na(logprice) & endbuyer) %>% summarise(mean = mean(logprice))
...
paint <- paint %>%
  ...
  mutate_at(vars(dealer, origin_author, origin_cat, school_pntg, 
                 diff_origin, artistliving, authorstyle, authorstandard,
...
paint_ <- paint %>% 
  dplyr::select(-c("sale", "lot", "diff_origin", "count","subject", "author",
                   ...
                   "winningbiddertype", "figures", "singlefig", "price","authorstandard"))
...
library(lars)
library(glmnet)
mm = model.matrix(~(.-1)^2, data = paint_train[,-5])
lasso = cv.glmnet(mm, paint_train[["logprice"]])
plot(lasso)

c1se = coef(lasso, s = "lambda.1se") %>% as.matrix
cmin = coef(lasso, s = "lambda.min") %>% as.matrix
table = cbind(cmin, c1se)
colnames(table) = c("lambda.min", "lambda.1se")
table = table[colSums(t(table))!=0,]
kable(table, caption = "coefficient values under different 
      penalizations chosen through cross validation. 
      0 coefficients for both models not shown.")
```

```{r}
bootstrap = function(data, fitfunct, testpoint, iter){
  inds = replicate(iter, sample.int(nrow(data), replace = T), simplify = F)
  resamp = lapply(inds, function(ind){data[ind,]})
  out = sapply(resamp, fitfunct, testpoint)
  return(out)
}

fitlasso = function(data, testpoint){
  ls = cv.glmnet(data[,-1], data[,1])
  out = predict(ls, testpoint, s = "lambda.min")
}

bootdat = cbind(paint_train[["logprice"]], mm)
test = model.matrix(~(.-1)^2, paint_test[,-5])

lasso.boot = bootstrap(bootdat, fitlasso, test, 1000)
```
