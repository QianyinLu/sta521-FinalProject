...
paint_train$authorstandard <- str_remove_all(paint_train$authorstandard, 
                                             paste(remove, collapse = "|")) %>% as.factor()
paint_test$authorstandard <- str_remove_all(paint_test$authorstandard, 
                                             paste(remove, collapse = "|")) %>% as.factor()
author_level <- c("A", "B", "C", "D")
paint_train_group <- 
  paint_train %>% 
  group_by(authorstandard) %>% 
  mutate(avg_price = mean(logprice)) %>% 
  arrange(desc(avg_price))
paint_train_group <- paint_train_group %>% mutate(level = case_when(avg_price >= quantile(paint_train_group$avg_price, 0.75) ~ author_level[1],
                           avg_price < quantile(paint_train_group$avg_price, 0.75) & avg_price >= quantile(paint_train_group$avg_price, 0.5) ~ author_level[2],
                           avg_price < quantile(paint_train_group$avg_price, 0.5) & avg_price >= quantile(paint_train_group$avg_price, 0.25) ~ author_level[3],
                           avg_price < quantile(paint_train_group$avg_price, 0.25) ~ author_level[4]))
```
...
treeWAu = randomForest(logprice ~ .,
data=paint_train,
mtry=5, importance =TRUE)
sqrt(ncol(paint_train))
