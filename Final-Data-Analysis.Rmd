...
# Fix position > 1
paintings_train$position[paintings_train$position > 1] <- paintings_train$position[paintings_train$position > 1]/100

# Clean Data
paintings_train <- paintings_train %>%
  mutate(dealer = as.factor(dealer),
         year = as.factor(year),
         ...
         winningbiddertype = as.factor(recode(na_if(winningbiddertype,""),
                                              .missing = 'NA')),
         endbuyer = as.factor(recode(na_if(endbuyer,''),
                                     .missing = 'NA')),
         Interm = recode(Interm,
                         .missing = 0,
                         `0` = 0,
                         `1` = 1),
         type_intermed = as.factor(recode(na_if(type_intermed,''),
                                          .missing='NA')),
         Shape = as.factor(recode(na_if(Shape,""),
                         .missing = 'Unknown',
                        'ovale' = 'oval',
                        'ronde' = 'round')),
         mat = as.factor(recode(na_if(mat,""),
                      .missing = 'NA',
                      'n/a' = 'NA')),
         materialCat = as.factor(recode(na_if(materialCat,""),
                                        .missing = 'NA')),
         material = as.factor(recode(na_if(material,""),
                                     .missing = 'NA',
                                     'n/a' = 'NA'))
         )

table(paintings_train$Shape)
```
### Comments after cleaning data:

What do we do with the NA's in Height, Width, Surface_Rect. Some of these are NA's because the shape of the painting is round. In that situation, we don't want to fill in values for the NA's, and we don't want to throw away the entire row. What are our other options... 

```{r To Do #3}
library(GGally)

ggpairs(paintings_train[,c("Height_in", "Width_in", "Surface_Rect", "Diam_in", "Surface_Rnd", "Surface", "position", "nfigures", "logprice")])
```

```{r To Do #4}
train.gathered <- paintings_train %>%
  dplyr::select(-c(sale,lot,position,price,count,subject,authorstandard,author,winningbidder,Height_in,Width_in,Surface_Rect,Diam_in,Surface_Rnd,Surface,nfigures)) %>%
  tidyr::gather(key = "variable", value = "value", -logprice)

ggplot(train.gathered, aes(x = value, y = logprice, colour = variable, group = value)) +
  facet_wrap(vars(variable), scales = "free_x") +
  guides(color = "none") +
  geom_boxplot(data = train.gathered)
```
