...
kable(ImpMeasure[order(-ImpMeasure$Overall),][15:19,], digits = 3)
...
From random forrest we obtained the 5 least important variables and discarded them in further modeling. The variables are displayed in the table above. 
...
```
From the marginal inclusion probability plot, we should exclude *materialCat*, *nfigures*, *lands_elem*, *year*:*dealer* since their margnial inclusion probability is less than 0.5. 
...
new_lm <- lm(logprice ~ year + dealer + origin_cat + diff_origin +
                         ...
                         lands_sc + lands_elem + othgenre + discauth + (year + authorstyle + discauth):dealer + year:discauth)
new <- lm(logprice ~ year + dealer + expensive + authorstyle + endbuyer + Interm + Surface + finished + lrgfont, data = paint_train)
avPlots(new_lm)
print(av)
plot(new)
vif(new)
