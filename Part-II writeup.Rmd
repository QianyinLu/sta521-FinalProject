...
From the Added variable plots above, we observed that the regression line is nearly flat with variables *paired*, *origin_cat*, *diff_origin* and *year*:*dealer*, so we deleted these variables to refit the linear model. Our final model can be summarised as follows: 
...
```{r Example}
lm1 <- lm(logprice ~ year + dealer + expensive + authorstyle + endbuyer + Interm + Surface + finished + lrgfont + diff_origin, data = TRAIN)
