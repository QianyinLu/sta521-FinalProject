...
paint <- paint %>%
  ...
  mutate(nfigures = as.factor(dplyr::recode(nfigures,
                                            `0` = 0,
                                            .default = 1))) %>% 
  dplyr::select(-c("sale", "lot", "count","subject", "author", "origin_author", "Shape", 
...
paint$year <- scale(paint$year, scale = F)
...
paint_test <- paint_test %>% 
  mutate(expensive = case_when(authorstandard %in% author_group[[1]] ~ level[1], 
                           authorstandard %in% author_group[[2]] ~ level[2]) %>% 
           as.factor)
paint_test$expensive[is.na(test_author$expensive)] <- level[2]
# summary(paint_train)
```
...
#         lower = list(continuous = wrap("points", alpha = 0.5, size = 0.3)))+
#   theme(axis.text = element_text(angle = 45, hjust = 1))
# ggpairs(paint_train, columns = c(5, 26:31), 
#         lower = list(continuous = wrap("points", alpha = 0.5, size = 0.3)))+
#   theme(axis.text = element_text(angle = 45, hjust = 1))
```
...
```{r}
full <- lm(logprice ~ (year + year_sq + dealer + origin_cat + diff_origin + authorstyle + 
    endbuyer + Interm + Surface + materialCat + nfigures +
    engraved + prevcoll + paired + finished + lrgfont + lands_sc + 
      lands_elem + othgenre + discauth + expensive)^2, data = paint_train)
summary(full)

full_aic <- step(full, trace = F, k = 2)
