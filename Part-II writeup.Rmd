...
full <- lm(logprice ~ (year + dealer + origin_cat + diff_origin + 
                         authorstyle + endbuyer + Interm + Surface + materialCat + 
                         nfigures + engraved + prevcoll + paired + finished + lrgfont + 
                         lands_sc + lands_elem + othgenre + discauth + expensive)^2 , data = paint_train)
model_ini <- lm(logprice ~ year + dealer + origin_cat + diff_origin + 
                         authorstyle + endbuyer + Interm + Surface + materialCat + 
                         nfigures + engraved + prevcoll + paired + finished + lrgfont + 
                         lands_sc + lands_elem + othgenre + discauth + expensive + (year + authorstyle + discauth):dealer + year:discauth, data = paint_train)
summary(full)

summary(lm(logprice ~ log(Surface), data = paint_train))
