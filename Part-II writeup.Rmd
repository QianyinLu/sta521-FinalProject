...
To check for the multicollinearity problem, we used variance inflation factor (VIF). The result is in the table below. Issue of multicollinearity is negligible for no VIF exceeds 5.  
```{r}
vif <- vif(final_model) %>% as.data.frame()
kable(vif, "latex", booktabs = T,
      digits = 3,caption = "VIF of final model") %>% 
  kable_styling(latex_options = "stripped", position = "center")
```
