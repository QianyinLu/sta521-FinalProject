...
- *history*, *original*, *type_intermed* and *pastorale* have serious **class imbalance** problem, where value of 1 is seldomly observed. So we will discard these variables in modeling.  
...
```{r model1, echo=TRUE}
model_ini = lm(logprice ~ year + origin_author + dealer + discauth + endbuyer + Interm + Surface + materialCat + diff_origin + engraved + prevcoll + origin_cat + paired + portrait + figures + finished + lrgfont + lands_sc + lands_elem, data=paint_train)
summary(model_ini)
