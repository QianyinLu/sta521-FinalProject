...
paint_preselect <- paint_preselect%>% 
  mutate(Surface = dplyr::recode(Surface,.missing = median(paint$Surface, na.rm = T)))
paint_preselect$Surface[paint_preselect$Surface==0] <- median(paint$Surface, na.rm = T)
# summary(paint_preselect)
paint_train <- paint_preselect%>%
  filter(!is.na(logprice))
paint_test <-paint_preselect%>%
  filter(is.na(logprice))
```
(need to add description to each variable deleted above, but it actually makes more sense to delete just a few of them before eda, then delete the rest looking at the eda, in this way we also do not need to plot the nfigures plot individually like above)
