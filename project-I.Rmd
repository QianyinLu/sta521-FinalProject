...
paint_train_trans_outlier = paint_train_trans[-c(218,1129),]
model1.1 <- lm(formula = logprice ~ dealer + year + origin_cat + artistliving + 
                   ...
                   Interm:lrgfont + Interm:discauth + Surface:discauth + 
                   prevcoll:finished + paired:finished + 
                   paired:lrgfont + paired:discauth, data = paint_train_trans_outlier)
# summary(model1.1)
```
...
predictions = as.data.frame(
  exp(predict(model2, newdata=paint_test_trans, 
              interval = "pred")))
