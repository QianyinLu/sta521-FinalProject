...
plot(lm_bas, which = 4)
x = predict(lm_bas, newdata = paint_test, set.fit = T)
fit <- x$fit
...
```{r}
library(party)
cf1 <- cforest(logprice ~ year + dealer + origin_cat  +  diff_origin +
                         authorstyle + endbuyer + Interm + Surface + materialCat + nfigures + engraved + prevcoll + paired + finished + lrgfont + lands_sc + lands_elem + othgenre + discauth, data= paint_train, control=cforest_unbiased(mtry=4))
varimp(cf1) # conditional=True, adjusts for correlation
```
