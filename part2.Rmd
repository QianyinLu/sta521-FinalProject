...
```{r Example}
lm1 <- lm(logprice ~ year + dealer + expensive+
                         authorstyle + endbuyer + Interm + Surface +  
                         finished + lrgfont, data = TRAIN)
...
model_ini <- lm(logprice ~ year + dealer + origin_cat + diff_origin + expensive + 
                         authorstyle + endbuyer + Interm + Surface + materialCat + 
                         nfigures + engraved + prevcoll + paired + finished + lrgfont + 
                         lands_sc + lands_elem + othgenre + discauth  + (year + authorstyle + discauth):dealer + year:discauth, data = TRAIN)
...
```
```{r}
lm1 <- lm(logprice ~ year + dealer + origin_cat + diff_origin + expensive + 
                         authorstyle + endbuyer + Interm + Surface + materialCat + 
                         nfigures + engraved + prevcoll + paired + finished + lrgfont + 
                         lands_sc + lands_elem + othgenre + discauth  + (year + authorstyle + discauth):dealer + year:discauth, data = TRAIN)
```
...
ImpMeasure[order(-ImpMeasure$Overall),][15:19,]


```

```{r}
lo<-lm(logprice~year + dealer + origin_cat + diff_origin + expensive+
                         authorstyle + endbuyer + Interm + Surface + materialCat + 
                          + finished + lrgfont + 
...
predictions = as.data.frame(
  exp(predict(lm_bas, newdata=paint_test, 
              interval = "pred")))
