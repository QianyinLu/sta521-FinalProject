...
kable(ImpMeasure[order(-ImpMeasure$Overall),][1:10,], digits = 3)
# training set 
...
```{r new linear model}
new <- lm(logprice ~ year + dealer + expensive + 
                         authorstyle + endbuyer + Interm + Surface +  
                         finished + lrgfont, data = paint_train)
avPlots(new_lm)
print(av)
plot(new)
vif(new)
...
```
```{r}
fitted <- exp(predict(new, newdata = paintings_validation))
Top10 <- cbind(paintings_validation, fitted) %>% arrange(desc(fitted)) %>% slice(1:10)
```
Using our model for predicting price for validation data set, we got our top 10 valuable paintings. We can see that these paintings share certain important features, such as they are all from the same dealer, R. In addition, endbuyers are mostly from category C and the dealer devotes an additional paragraph etc.

```{r}
predictions = as.data.frame(
  exp(predict(new, newdata=paint_test, 
              interval = "pred")))
...
coverage.plot <- function(output, rmse, conf.level = 0.95, model) {
  ...
  ggplot(output, aes(x = fit, y = response)) +
    ...
    annotate("text", x = 1.75, y = 10.05, label = paste("RMSE:", rmse),
  parse = TRUE) +
    annotate("text", x = 1.75, y = 9.35, label = paste("Coverage: ", mean(output$covered)*100, "%")) +
    annotate("text", x = 2.1, y = 10.75, label = paste("Model:", model))
  }
...
```{r Example}
lm1 <- lm(logprice ~ year + materialCat + expensive + endbuyer + lrgfont + dealer
          + authorstyle*endbuyer + discauth*lrgfont, data = TRAIN)
RMSE_lm <- rmse(exp(predict(lm1, newdata = VALIDATE)), exp(VALIDATE$logprice)) # on new data
rmse(exp(predict(lm1)), exp(TRAIN$logprice)) # on seen data
cov_data <- coverage.test(lm1, VALIDATE, VALIDATE$logprice)
coverage.plot(cov_data, RMSE_lm, model = "Linear Regression")
```
...
rfdf <- rfdf %>%
    mutate(covered = (response >= lower & response <= upper))
RMSE.rf <- rmse(exp(predict(rf, newdata = VALIDATE)), exp(VALIDATE$logprice))
coverage.plot(rfdf, RMSE.rf, model = "Random Forest")
```
...
ints <- ints %>%
    mutate(covered = (response >= lower & response <= upper))
RMSE.bma <- rmse(exp(predict(lm_bas, newdata = VALIDATE)$fit), exp(VALIDATE$logprice))
coverage.plot(ints, RMSE.bma, model = "BMA")
```
